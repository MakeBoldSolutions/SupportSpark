...
beforeAll(async () => {
  ...
  app.use(((err, _req, res, _next) => res.status(err.status || 400).json({ message: err.message })) as express.ErrorRequestHandler);
  for (const [agent, address] of [[owner, ownerEmail], [supporter, supporterEmail], [outsider, email("outsider")]] as const) {
    await agent.post("/api/register").send({ email: address, password: "Testing123!", firstName: "Test", lastName: "User" }).expect(201);
  }
  const result = await owner.post("/api/conversations").send({ title: "Private journey", initialMessage: "First update" }).expect(201);
  id = result.body.id;
  initialId = result.body.data.messages[0].id;
});
afterAll(async () => { await new Promise<void>((resolve) => server.close(() => resolve())); await fs.rm(root, { recursive: true, force: true }); });
...
it("rejects anonymous and unrelated users for conversation reads and writes", async () => {
  ...
  await outsider.post(`/api/conversations/${id}/messages`).send({ content: "Intrusion" }).expect(403);
...
it("enforces invitation ownership and acceptance before access", async () => {
  ...
  await owner.post("/api/supporters/invite").send({ email: email("missing") }).expect(404);
  const invitation = await owner.post("/api/supporters/invite").send({ email: supporterEmail }).expect(201);
  ...
  await outsider.patch(`/api/supporters/${invitationId}/status`).send({ status: "accepted" }).expect(404);
  ...
  await supporter.patch(`/api/supporters/${invitationId}/status`).send({ status: "accepted" }).expect(200);
  ...
  expect((await owner.get("/api/supporters").expect(200)).body.mySupporters[0].supporterEmail).toBe(supporterEmail);
});
it("preserves simultaneous replies through the actual HTTP API", async () => {
  await Promise.all(Array.from({ length: 8 }, (_, i) => (i % 2 ? owner : supporter)
    .post(`/api/conversations/${id}/messages`).send({ content: `Reply ${i}`, parentMessageId: initialId }).expect(200)));
  ...
  await owner.post(`/api/conversations/${id}/messages`).send({ content: "Nested", parentMessageId: replies[0].id, images: ["/photo.png"] }).expect(200);
  await owner.post(`/api/conversations/${id}/messages`).send({ content: "Missing parent", parentMessageId: "missing" }).expect(404);
...
it("revokes access when the supporter rejects the relationship", async () => {
  await supporter.patch(`/api/supporters/${invitationId}/status`).send({ status: "rejected" }).expect(200);
  await supporter.get(`/api/conversations/${id}`).expect(403);
