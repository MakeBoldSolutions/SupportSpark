...
it("updates, dismisses and removes toast notifications", () => {
  ...
  act(() => {
    notification = toast({ title: "Saved" });
  });
  expect(result.current.toasts[0].title).toBe("Saved");
  act(() => {
    notification.update({ id: notification.id, title: "Updated" });
  });
...
it("keeps unrelated notifications when updating or removing by id", () => {
  ...
  expect(
    reducer(state, { type: "UPDATE_TOAST", toast: { id: "a", title: "New" } }).toasts[1].title
  ).toBe("B");
  ...
  expect(reducer(state, { type: "REMOVE_TOAST", toastId: "a" }).toasts.map((t) => t.id)).toEqual([
    "b",
  ]);
