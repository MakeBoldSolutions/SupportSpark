...
vi.mock("framer-motion", async (original) => ({ ...await original<typeof import("framer-motion")>(), AnimatePresence: ({ children }: { children: ReactNode }) => children }));
...
beforeEach(() => { auth.user = null; window.history.replaceState({}, "", "/"); vi.stubGlobal("scrollTo", vi.fn()); vi.stubGlobal("__APP_VERSION__", "test"); vi.stubGlobal("__BUILD_DATE__", "2026-09-21"); });
afterEach(() => { vi.restoreAllMocks(); vi.unstubAllGlobals(); vi.useRealTimers(); });
it("offers public entry points and an authenticated dashboard link", () => {
  const view = render(<Home />);
  expect(screen.getByRole("button", { name: "Start Your Journey" }).closest("a")).toHaveAttribute("href", "/auth");
  auth.user = { id: "owner", email: "owner@example.com", password: "unused", firstName: "Jane" };
  view.rerender(<Home />);
  expect(screen.getByRole("button", { name: "Go to Your Dashboard" }).closest("a")).toHaveAttribute("href", "/dashboard");
});
it("rotates the home-page quote over time", async () => {
  vi.useFakeTimers(); const view = render(<Home />); const before = view.container.textContent;
...
it("provides mobile sign-in for visitors", async () => {
  ...
  expect(screen.getByRole("button", { name: "Sign In" }).closest("a")).toHaveAttribute("href", "/auth");
  await userEvent.click(screen.getByRole("button", { name: "Sign In" }));
});
it("navigates the demo tour in both directions and restarts it", async () => {
  render(<QueryClientProvider client={new QueryClient()}><Demo /></QueryClientProvider>);
...
it.each(["patient", "supporter"])("handles demo %s entry and missing demo data", async (role) => {
  const login = vi.spyOn(storage, "loginDemoSupporter").mockReturnValue({ id: "demo", email: "demo@example.com", password: "unused" });
  render(<QueryClientProvider client={new QueryClient()}><Demo /></QueryClientProvider>);
  await userEvent.click(screen.getByTestId(`button-login-${role}`));
  expect(window.location.pathname).toBe("/dashboard");
  login.mockImplementation(() => { throw new Error("missing"); });
  await userEvent.click(screen.getByTestId(`button-login-${role}`));
  expect(window.location.pathname).toBe("/auth");
});
it("renders a not-found page", () => { render(<NotFound />); expect(screen.getByText(/404/)).toBeInTheDocument(); });
