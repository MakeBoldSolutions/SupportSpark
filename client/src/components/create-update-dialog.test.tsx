...
beforeEach(() => {
  mutation.mutateAsync.mockResolvedValue({ id: 1 });
...
it("validates and publishes a text update", async () => {
  ...
  await userEvent.click(screen.getByRole("button", { name: "Publish Update" }));
  await waitFor(() =>
    expect(mutation.mutateAsync).toHaveBeenCalledWith({ title: "Journey", initialMessage: "News" })
  );
...
it("formats text and uploads only valid selected images", async () => {
  vi.stubGlobal(
    "fetch",
    vi.fn().mockResolvedValue({ ok: true, json: async () => ({ images: ["/image.png"] }) })
  );
  await open();
  await fill();
  for (const button of ["bold", "italic", "link", "add-images"])
    await userEvent.click(screen.getByTestId(`button-${button}`));
  fireEvent.change(screen.getByTestId("input-image-upload"), {
    target: {
      files: [
        new File(["image"], "image.png", { type: "image/png" }),
        new File(["html"], "file.html", { type: "text/html" }),
      ],
    },
  });
