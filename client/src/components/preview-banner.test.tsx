...
vi.mock("@/lib/local-storage-adapter", () => ({ storage: { getCurrentUser: () => state.authenticated ? { id: "owner" } : null, getStorageUsagePercent: async () => state.usage, resetAllData: state.reset } }));
afterEach(() => { vi.unstubAllGlobals(); });
it("shows local-storage disclosure without reset for visitors", () => {
  state.authenticated = false; state.usage = 0; render(<PreviewBanner />);
...
it("warns about storage capacity and resets signed-in preview data", async () => {
  state.authenticated = true; state.usage = 90;
