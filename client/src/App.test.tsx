...
vi.mock("@/hooks/use-conversations", () => ({
  ...
  useCreateConversation: () => ({ isPending: false }),
  useConversation: () => ({ data: undefined }),
  useAddMessage: () => ({}),
}));
vi.mock("@/hooks/use-supporters", () => ({
  useSupporters: () => ({ data: undefined }),
  useInviteSupporter: () => ({}),
}));
vi.mock("@/lib/local-storage-adapter", () => ({
  storage: { getCurrentUser: () => null, getStorageUsagePercent: async () => 0 },
}));
beforeEach(() => {
...
it("requires login before rendering a protected route", async () => {
  ...
  state.isLoading = false;
  state.user = { id: "owner", email: "owner@example.com", password: "unused", firstName: "Jane" };
  view.rerender(<App />);
  ...
  await act(async () => {
    window.location.hash = "#/conversation/1";
  });
